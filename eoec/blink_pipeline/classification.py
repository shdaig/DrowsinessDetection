...
def _extract_features_labels(file_name: str, sfreq: int, channels: list[str],
    ...
    eo_signal_chunks = eoec.get_long_signal(raw, channels=channels, samples=eo_samples, depth=signal_depth_seconds * sfreq)
    ec_signal_chunks = eoec.get_long_signal(raw, channels=channels, samples=ec_samples, depth=signal_depth_seconds * sfreq)
    eo_labels = np.full((eo_signal_chunks.shape[0],), 1)
    ec_labels = np.full((ec_signal_chunks.shape[0],), 0)
    print(eo_signal_chunks.shape)
    eo_signal_chunks = eo_signal_chunks.mean(axis=1)
    ec_signal_chunks = ec_signal_chunks.mean(axis=1)
...
if __name__ == "__main__":
    ...
    subjs = ["egipko", "borovensky", "dshepelev", "golenishev", "azhogin"]
    year = "2023"
    sfreq = 500
        ...
        for k in range(1):
            ...
            for test_idx in eoec_dataset:
                ...
                with tf.device('/GPU:1'):
                    ...
                    x_train = x_train.reshape(x_train_shape_0 * x_train_shape_1, x_train.shape[2])
                    x_test = x_test.reshape(x_test_shape_0 * x_test_shape_1, x_test.shape[2])
