...
from drowsiness.utils.color_print import *
...
def get_sleep_state_from_file(raw: mne.io.Raw, labels_file: str) -> np.ndarray:
    ...
    ends = []
    sample_rate = 500
    with open(os.path.join("video_labels", labels_file), 'r') as f:
        lines = f.read().split('\n')
        for line in lines:
...
if __name__ == "__main__":
    ...
    for test_file in subject_files:
        ...
        autoencoder_train_dataset = autoencoder_train_dataset.batch(batch_size=512)
        ...
        autoencoder.fit(autoencoder_train_dataset,
                        epochs=50, verbose=2,
        ...
        os.mkdir(os.path.join(models_dir, test_file))
        autoencoder.save_weights(os.path.join(models_dir, test_file, f"{test_file}_ae.weights"))
